...
void check_distribtion()
{
	//-----------------User Input--------------------//
	//Enter filenames
	//Make sure file1 is low pileup and file2 is high pileup, or change the legend
	//TString sigName1="/afs/cern.ch/work/e/ebusch/public/h_bbar_datasets/high_pilueup_new_stats/all_mass_points.root";
	//TString sigName2="/afs/cern.ch/work/e/ebusch/public/h_bbar_datasets/high_pilueup_new_stats/all_mass_points.root";
        //TString bkgName1="/afs/cern.ch/work/e/ebusch/public/h_bbar_datasets/jetntuple_local/user.ebusch.mc16_13TeV.361026._jetjet_JZ6W..194706944.jblu_0730_out1.root/user.ebusch.14762904._000001.out1.root";
        //TString bkgName2="/afs/cern.ch/work/e/ebusch/public/h_bbar_datasets/jetntuple_local/user.ebusch.mc16_13TeV.361026._jetjet_JZ6W..194706944.jblu_0730_out1.root/user.ebusch.14762904._000001.out1.root";


        TString sigName1="/afs/cern.ch/work/e/ebusch/public/h_bbar_datasets/jetntuple_local/all_mass_points.root_lowptcut2.root";
        TString sigName2="/afs/cern.ch/work/e/ebusch/public/h_bbar_datasets/jetntuple_local/all_mass_points.root_lowptcut2.root";
        TString bkgName1="/afs/cern.ch/work/e/ebusch/public/h_bbar_datasets/jetntuple_local/user.ebusch.14762904._000001.out1.root_lowptcut2.root";
        TString bkgName2="/afs/cern.ch/work/e/ebusch/public/h_bbar_datasets/jetntuple_local/user.ebusch.14762904._000001.out1.root_lowptcut2.root";
	...
	const char * varNames[numVars] = {"jet_pt","jet_eta"}; //, "jet_m"};
	...
	for (Int_t iter = 0; iter <numVars; iter++){
		...
		TString bkghTitle1;
                TString bkghTitle2;
		sighTitle1.Form("%s_(1300-1800GeV)_sig",varNames[iter]);
		sighTitle2.Form("%s_(1300-1800GeV)_sig",varNames[iter]);
                bkghTitle1.Form("%s_(1300-1800GeV)_bkg",varNames[iter]);
                bkghTitle2.Form("%s_(1300-1800GeV)_bkg",varNames[iter]);
		...
		sigHist1[iter] = new TH1F(sighTitle1,sighTitle1,30,minuse,maxuse);
		sigHist2[iter] = new TH1F(sighTitle2,sighTitle2,30,minuse,maxuse);
		bkgHist1[iter] = new TH1F(bkghTitle1,bkghTitle1,30,minuse,maxuse);
                bkgHist2[iter] = new TH1F(bkghTitle2,bkghTitle2,30,minuse,maxuse);
	}
	...
	for(Int_t iter = 0; iter <numVars; iter++){
		for(Long64_t iEntry1 = 0; iEntry1 <numsigEvents1; iEntry1++){
			...
			//Double_t sigScale1 = sigmcwg1*sigFilter_ef1/sigEvents1;		
	...
	for(Int_t iter = 0; iter <numVars; iter++){
			...
			Double_t sigScale2 = sigmcwg2*sigCross_Section2*sigFilter_ef2/sigEvents2;
                        //Double_t sigScale2 = sigmcwg2*sigFilter_ef2/sigEvents2;
			sigHist2[iter]->Fill(sigVarPointers2[iter],sigScale2);
                }
        }
        ...
        for(Int_t iter = 0; iter <numVars; iter++){
			...
			//Double_t bkgScale1 = bkgmcwg1*bkgFilter_ef1/bkgEvents1;
        ...
        for(Int_t iter = 0; iter <numVars; iter++){
			...
			//Double_t bkgScale2 = bkgmcwg2*bkgFilter_ef2/bkgEvents2;
	...
	for(Int_t iter = 0; iter <numVars; iter++){
		...
		sigHist1[iter]->SetLineColor(kRed);
		...
		sigHist2[iter]->GetYaxis()->SetTitle("A.U.");
                ...
                bkgHist1[iter]->SetLineColor(kBlue);//(38);
                bkgHist2[iter]->SetLineColor(kBlue);//(46);
		...
		//legend[iter]->AddEntry(sigHist1[iter],"Sig M300-M400");
		legend[iter]->AddEntry(sigHist2[iter],"Sig M300-M2750");
		//legend[iter]->AddEntry(bkgHist1[iter],"Bkg JZ6W");
		legend[iter]->AddEntry(bkgHist2[iter],"Bkg JZ6W");
		legend[iter]->Draw();
		//c[iter]->BuildLegend();
